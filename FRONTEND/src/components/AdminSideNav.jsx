...
const SideNav = ({ setVistaActual }) => {
    ...
    return (
        <div className={`left-column top-0 left-0 h-screen bg-gray-900 text-white p-4 ${isOpen ? 'w-64' : 'w-16'} duration-300 ease-in-out`}>
            <button 
            ...
            <div className="flex flex-col justify-between h-full pb-6">
                <div className="flex flex-col gap-4 mt-2">
                    ...
                    >
                        <FaUsers /> {isOpen && <span>Gestión de Plan de Estudios</span>}
                    </button>
                    <button 
                ...
                <div className="flex flex-col gap-2">
                    <Button 
                        ...
                        className={`flex items-center gap-2 bg-orange-500 hover:bg-orange-600 rounded-md p-2 ${isOpen ? 'w-full' : 'w-12'}`}
                    >
                        <FaSignOutAlt className="text-white" /> {isOpen && <span>Cerrar sesión</span>}
                    </Button>
                    <button 
