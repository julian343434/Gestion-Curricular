    ...
    <div className="w-full min-h-screen bg-gray-50">
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section aria-labelledby="carreras-heading" className="mb-12">
          <h2 id="carreras-heading" className="text-3xl font-bold mb-6  text-black text-left">
            Carreras
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {currentCarreras.map((carrera, index) => (
              <div key={index} className="bg-white rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 overflow-hidden">
                <div className="p-6">
                  <h3 className="text-2xl font-bold mb-4 text-black">{carrera.nombre}</h3>
                  <ul className="space-y-2 text-black text-left">
                    <li><strong>Créditos:</strong> {carrera.creditos}</li>
                    <li><strong>Duración:</strong> {carrera.duracion}</li>
                    <li><strong>{carrera.codigo}</strong></li>
                    <li>{carrera.registro}</li>
                    {carrera.acreditacion && <li className="text-green-600">{carrera.acreditacion}</li>}
                  </ul>
                </div>
          ...
          {filteredCarreras.length > itemsPerPage && (
            <div className="mt-8 flex justify-center">
              ...
              {Array.from({ length: Math.ceil(filteredCarreras.length / itemsPerPage) }, (_, i) => (
                <button
                  key={i}
        ...
        <section aria-labelledby="facultades-heading">
          <h2 id="facultades-heading" className="text-3xl font-bold mb-6 text-left text-black">
            Nuestras Facultades
          </h2>
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {facultades.map((facultad, index) => (
              <div key={index} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow duration-300 p-4">
                <h3 className="font-semibold text-lg mb-2 text-black">{facultad.nombre}</h3>
